...
with tf.name_scope('fccnn_Layer'):
	...
	h_fccnn4 = tf.nn.relu(util.atrous_conv2d(h_pool3, W_fccnn1, valid=True, rate=8))  # Perform convolution (with zero padding) and apply ReLU.
	
# Insert more FC layers here.

with tf.name_scope('Output_Layer'):
	# Now add a final sigmoid layer for prediction of 0-1 probability and readout.
	W_fccnn5 = util.weight_variable([1, 1, firstFCNeurons, 2], "w_fccnn_5")
...
with tf.name_scope('XEnt'):
	cross_entropy = tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(targets=y_syn, logits=y_syn_logit_flat, pos_weight=(1 / true_ratio_syn), name='syn_Loss'))
...
if training:
	...
	for i in range(trainingSteps):
		if i % valRegularity == 0:
			...
			for j in range(validationImages):
				for k in range(8):
					...
					val_cross_entropy[j, k], val_accuracy[j, k], val_fmeasure[j, k] = both.eval(feed_dict={x: reshaped, y_syn: valBatch[1]['SYN']}) # Removed dropout , keep_prob: 1.0})
		...
		_, summary = sess.run([train_step, summary_op], feed_dict={x: batch[0], y_syn: batch[1]}) # removed dropout , keep_prob: dropoutProb})
...
def apply_classifier(_func, _images):
	...
	for i in range(_numFrames):
		...
		startTime = timeit.default_timer()
		predFlat = _func.eval(feed_dict={x: singleIm}) # removed dropout. , keep_prob: 1.0})
		elapsed = timeit.default_timer() - startTime
		appTimes[i] = elapsed
...
#		load_path = fileOutputName
	...
	if deployTrain:
		print("Beginning dense application to training set.")
